      ...
      <View className="flex-1 px-5 pt-4">
        ...
        <View className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            className={`
              py-4 rounded-xl items-center justify-center shadow-sm mb-3
              ${controller.isRecording ? 'bg-red-500' : 'bg-blue-600'}
              ${!controller.canRecord && !controller.isRecording ? 'opacity-50' : 'opacity-100'}
            `}
          >
            <Text className="text-white font-bold text-lg tracking-wide uppercase">
              {controller.isRecording ? 'Stop & Salva' : 'Registra Nota'}
            </Text>
          </TouchableOpacity>
        </View>
